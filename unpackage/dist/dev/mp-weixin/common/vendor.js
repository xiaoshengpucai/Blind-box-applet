...
function initRuntimeSocketService() {
  const hosts = "198.18.0.1,192.168.0.104,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_PXlgji";
...
const mixin = defineMixin({
  ...
  methods: {
    ...
    $uGetRect(selector, all2) {
      return new Promise((resolve2) => {
        index$1.createSelectorQuery().in(this)[all2 ? "selectAll" : "select"](selector).boundingClientRect((rect) => {
...
function buildURL$3(url2, params) {
  if (!params) {
    return url2;
  }
  let serializedParams;
  if (isURLSearchParams$2(params)) {
    serializedParams = params.toString();
  } else {
    const parts = [];
    forEach$2(params, (val, key) => {
...
const adapter = (config2) => new Promise((resolve2, reject) => {
  const fullPath = buildURL$3(buildFullPath$3(config2.baseURL, config2.url), config2.params);
  const _config = {
    url: fullPath,
    ...
    complete: (response) => {
      ...
      settle$3(resolve2, reject, response);
    }
  };
  let requestTask;
...
const dispatchRequest$1 = (config2) => adapter(config2);
...
function setConfig$1(configs) {
  ...
  shallowMerge(props$6, configs.props || {});
...
const resolveConfig = (config2) => {
  ...
  newConfig.url = buildURL$2(buildFullPath$2(newConfig.baseURL, newConfig.url, newConfig.allowAbsoluteUrls), config2.params, config2.paramsSerializer);
  if (auth) {
    headers.set(
      "Authorization",
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type) => {
    !resolvers[type] && (resolvers[type] = utils$4.isFunction(res[type]) ? (res2) => res2[type]() : (_, config2) => {
...
validators$1.spelling = function spelling(correctSpelling) {
  return (value, opt) => {
    index$1.__f__("warn", "at node_modules/axios/lib/helpers/validator.js:58", `${opt} is likely a misspelling of ${correctSpelling}`);
    return true;
  };
};
...
class Axios {
  ...
  getUri(config2) {
    config2 = mergeConfig(this.defaults, config2);
    const fullPath = buildFullPath$2(config2.baseURL, config2.url, config2.allowAbsoluteUrls);
...
var enhanceError$1 = function enhanceError(error2, config2, code2, request, response) {
...
var createError$1 = function createError(message, config2, code2, request, response) {
  var error2 = new Error(message);
  return enhanceError2(error2, config2, code2, request, response);
};
...
function transformResponse2(mpResponse, config2, mpRequestOption) {
...
function transformError(error2, reject, config2) {
  switch (platFormName) {
    case "wechat":
      if (error2.errMsg.indexOf("request:fail abort") !== -1) {
        reject(createError$2("Request aborted", config2, "ECONNABORTED", ""));
      } else if (error2.errMsg.indexOf("timeout") !== -1) {
        reject(createError$2("timeout of " + config2.timeout + "ms exceeded", config2, "ECONNABORTED", ""));
      } else {
        reject(createError$2("Network Error", config2, null, ""));
      }
      break;
    case "dd":
    case "alipay":
      if ([14, 19].includes(error2.error)) {
        reject(createError$2("Request aborted", config2, "ECONNABORTED", "", error2));
      } else if ([13].includes(error2.error)) {
        reject(createError$2("timeout of " + config2.timeout + "ms exceeded", config2, "ECONNABORTED", "", error2));
      } else {
        reject(createError$2("Network Error", config2, null, "", error2));
      }
      break;
    case "baidu":
      reject(createError$2("Network Error", config2, null, ""));
...
function transformConfig(config2) {
  var _a;
  if ([
    "alipay",
    "dd"
    ...
    config2.headers = config2.header;
    delete config2.header;
    if ("dd" === platFormName && config2.method !== "GET" && ((_a = config2.headers) === null || _a === void 0 ? void 0 : _a["Content-Type"]) === "application/json" && Object.prototype.toString.call(config2.data) === "[object Object]") {
...
function mpAdapter(config2, _a) {
  ...
  return new Promise(function(resolve2, reject) {
    ...
    var requestMethod = config2.method && config2.method.toUpperCase() || "GET";
    var mpRequestOption = {
      method: requestMethod,
      url: buildURL$1(buildFullPath$1(config2.baseURL, config2.url), config2.params, config2.paramsSerializer),
      timeout: config2.timeout,
      // Listen for success
      success: function(mpResponse) {
        var response = transformResponse2(mpResponse, config2, mpRequestOption);
        settle$1(resolve2, reject, response);
      },
      // Handle request Exception
      fail: function(error2) {
        transformError(error2, reject, config2);
      },
      complete: function() {
        requestTask = void 0;
      }
    ...
    if (config2.auth) {
      var _a2 = [config2.auth.username || "", config2.auth.password || ""], username = _a2[0], password = _a2[1];
    ...
    if (config2.responseType) {
      mpRequestOption.responseType = config2.responseType;
    }
    if (config2.cancelToken) {
...
const props$4 = defineMixin({
  props: {
    ...
    show: {
      type: Boolean,
      default: () => props$6.popup.show
    },
    // 是否显示遮罩
    overlay: {
      type: Boolean,
      default: () => props$6.popup.overlay
    },
    // 弹出的方向，可选值为 top bottom right left center
    mode: {
      type: String,
      default: () => props$6.popup.mode
    },
    // 动画时长，单位ms
    duration: {
      type: [String, Number],
      default: () => props$6.popup.duration
    },
    // 是否显示关闭图标
    closeable: {
      type: Boolean,
      default: () => props$6.popup.closeable
    },
    // 自定义遮罩的样式
    overlayStyle: {
      type: [Object, String],
      default: () => props$6.popup.overlayStyle
    },
    // 点击遮罩是否关闭弹窗
    closeOnClickOverlay: {
      type: Boolean,
      default: () => props$6.popup.closeOnClickOverlay
    },
    // 层级
    zIndex: {
      type: [String, Number],
      default: () => props$6.popup.zIndex
    },
    // 是否为iPhoneX留出底部安全距离
    safeAreaInsetBottom: {
      type: Boolean,
      default: () => props$6.popup.safeAreaInsetBottom
    },
    // 是否留出顶部安全距离（状态栏高度）
    safeAreaInsetTop: {
      type: Boolean,
      default: () => props$6.popup.safeAreaInsetTop
    },
    // 自定义关闭图标位置，top-left为左上角，top-right为右上角，bottom-left为左下角，bottom-right为右下角
    closeIconPos: {
      type: String,
      default: () => props$6.popup.closeIconPos
    },
    // 是否显示圆角
    round: {
      type: [Boolean, String, Number],
      default: () => props$6.popup.round
    },
    // mode=center，也即中部弹出时，是否使用缩放模式
    zoom: {
      type: Boolean,
      default: () => props$6.popup.zoom
    },
    // 弹窗背景色，设置为transparent可去除白色背景
    bgColor: {
      type: String,
      default: () => props$6.popup.bgColor
    },
    // 遮罩的透明度，0-1之间
    overlayOpacity: {
      type: [Number, String],
      default: () => props$6.popup.overlayOpacity
    }
  }
});
...
const props$2 = defineMixin({
  props: {
    bgColor: {
      type: String,
      default: () => props$6.statusBar.bgColor
    },
    // 状态栏获取得高度
    height: {
      type: Number,
      default: () => props$6.statusBar.height
    }
  }
});
