<template>
	<view class="navigation-container" :style="navigationStyle" @touchmove.stop.prevent="() => {}">
		<view v-for="(navItem, index) in navigationList" :key="navItem.id" class="navigation-item"
			:class="navigationItemClasses(index)" :style="navigationItemStyle(index)"
			@click="handleNavigationClick(navItem)">
			<image class="navigation-icon" :src="navItem.src" :alt="navItem.text" mode="aspectFit" lazy-load
				@load="handleImageLoad(navItem, index)" @error="handleImageError(navItem, index)" />
		</view>
	</view>
</template>
