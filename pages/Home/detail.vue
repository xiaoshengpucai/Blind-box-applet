...
onLoad(async (option) => {
	...
	categoryPrice.value = option.price
	try {
...
watch(categoryPrice, (newVal) => {
	raffleList.value = raffleList.value.map(item => {
		// 重新计算每个选项的 price，处理计算精度问题
		const price = Number((item.multiple * Number(newVal)).toFixed(2));
		return {
			...item,
			price
		}
